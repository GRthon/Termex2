...
ask_ques = "**يرجـى اختيـار أحد الجلسـات الآتيـة**"
buttons_ques = [
    [
        InlineKeyboardButton("بايروجـرام", callback_data="pyrogram"),
        InlineKeyboardButton("تيـليثـيون", callback_data="telethon"),
    ],
]
...
async def generate_session(bot: Client, msg: Message, telethon=False, is_bot: bool = False):
    if telethon:
        ...
        ty = "بايروجـرام"
    await msg.reply(f"** استخـراج سيشن **{ty}** بواسطـة 𝗚𝗥 جـارٍ..**")
    user_id = msg.chat.id
    ...
    if not is_bot:
        t = "**يـرجـى إرسـال رقـم هاتفـك مـع رمـز الدولةn/مثــال 📱: +9647700000000**"
    else:
    ...
    text = f"**هذا هو {ty} كـود سيشن** \n\n`{string_session}` \n\n** **ملاحظـة :** لا تشارك الكود لأحـد لأنـه يستطيع اختراق حسابك من خلاله ولا تنسى الانضمام بقناة السورس @G_Rthon"
    try:
        if not is_bot:
    ...
    await bot.send_message(msg.chat.id, " تم استخـراج {} كود سيشن.\n\nيرجـى تفقـد الرسائـل المحفوظـة ! \n\n**مستخـرج مـن** @G_Rthon".format("تيـليثـيون" if telethon else "بايروجـرام"))
